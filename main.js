...
const storedData = JSON.parse(localStorage.getItem("saveToLocalStorage")) || {
  ...
  tasks: [
    {
      text: "Buy Bananas for the pancakes",
      category: "Groceries",
      checked: true,
      liked: false,
    },
...
function updateTaskCountDisplay() {
  ...
  category.forEach((categorie) => {
    ...
    taskItem.innerHTML = `
            <button class="category" data-category="${capitalizeFirstLetter(
              categorie.name
            )}">
...
function addItem(type, text, categoryName) {
  if (type === "task") {
    ...
    let categoryToUpdate = category.find(
...
newCategoryForm.addEventListener("submit", function (event) {
  ...
  let existingCategory = category.find(
...
taskForm.addEventListener("submit", function (event) {
  ...
  if (newTaskText && newTaskCategory) {
    if (
      !tasks.some(
        (task) => task.text === newTaskText && task.category === newTaskCategory
      )
    ) {
  ...
  if (filteredTasks.length === 0) {
    ...
    filteredTasks.forEach((task) => {
      ...
      listItem.innerHTML = `
            ...
            }; color: ${task.checked ? "#EB5757" : "inherit"};">
                <b>${task.text}</b>
            </p>
            <span class="task-category" style="background-color: ${
              categoryColors[task.category] || "#000000"
            }">
                ${task.category}
            </span>
            <button type="button" class="btn-heart">
                <i class="${
                  task.liked ? "fa-solid" : "fa-regular"
                } fa-heart"></i>
            </button>
      ...
      removeButton.addEventListener("click", function () {
        ...
        if (confirmed) {
          ...
          let favoritesCategory = category.find(
            (cat) => cat.name === "Favorites"
          );
      ...
      heartButton.addEventListener("click", function () {
        ...
        heartButton.querySelector("i").className = task.liked
          ? "fa-solid fa-heart"
          : "fa-regular fa-heart";
...
toggleSearch.addEventListener("click", () => {
  ...
  if (searchForm.style.width == "0%" || searchForm.style.width == "") {
    ...
    filterInput.value = ""; // Réinitialiser le champ de saisie
  }
});
// Ajoutez un gestionnaire d'événements pour le champ de recherche
filterInput.addEventListener("input", function () {
  displayTasks(title.textContent == "All Tasks" ? null : title.textContent);
});
